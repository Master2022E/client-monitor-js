import {Browser, Certificate, ClientSample, DataChannel, Engine, ExtensionStat, IceCandidatePair, IceLocalCandidate, IceRemoteCandidate, InboundAudioTrack, InboundVideoTrack, MediaCodecStats, MediaDevice, MediaSourceStat, OperationSystem, OutboundAudioTrack, OutboundVideoTrack, PeerConnectionTransport, Platform, W3CStats as W3C,} from '@observertc/monitor-schemas';
...
export type SamplerConfig = {
   ...
   *
   * If server side componet is used to collect the samples, this parameter is
   * the critical to provide to match clients being in the same room.
   *
  ...
  /**
   * The identifier of the client. If it is not provided, then it a UUID is
   * generated. If it is provided it must be a valid UUID.
   *
   * DEFAULT: a generated unique value
   */
  clientId?: string;
  /**
   * the identifier of the call between clients in the same room. If not given
   * then the server side assigns one. If it is given it must be a valid UUID.
   *
   * DEFAULT: undefined
   */
  callId?: string;
  ...
  /**
   * Indicate if the sampler should use the timestamp provided by the rtcstats
   * (the minimum of the peer connection stats will be used as the timestamp of
   * the sample)
   */
...
export class Sampler {
  public static builder(): Builder {
    ...
    const result: Builder = {
      ...
      build(): Sampler {
        ...
        const appliedConfig: SamplerConstructorConfig =
            Object.assign(supplyDefaultConfig(), config);
  ...
  public addTrackRelation(trackRelation: TrackRelation): void {
    ...
    logger.debug(
        `Track relation for trackId: ${trackRelation.trackId} is set`,
        trackRelation);
  }

  public removeTrackRelation(trackId: string): void {
    logger.debug(
        `Track relation for trackId: trackId: ${trackId} is removed`,
        this._trackRelations.get(trackId));
  ...
  public make(): ClientSample|undefined {
    ...
    for (const peerConnection of this._statsReader.peerConnections()) {
      for (const [inboundAudioTrack, inboundVideoTrack] of this
      ...
      for (const [outboundAudioTrack, outboundVideoTrack] of this
      ...
      for (const iceCandidatePair of this._makeIceCandidatePair(
               peerConnection)) {
      ...
      for (const iceLocalCandidate of this._makeIceLocalCandidate(
               peerConnection)) {
        if (!iceLocalCandidates) iceLocalCandidates = [];
        iceLocalCandidates.push(iceLocalCandidate);
      }

      for (const iceRemoteCandidate of this._makeIceRemoteCandidate(
               peerConnection)) {
  ...
  private *
      _makeOutboundTrack(peerConnection: PeerConnectionEntry): Generator<
          [OutboundAudioTrack | undefined, OutboundVideoTrack|undefined], void,
          undefined> {
    for (const outboundRtp of peerConnection.outboundRtps()) {
      ...
      const remoteInboundRtpStats =
          outboundRtp.getRemoteInboundRtp()?.stats || {};
      const mediaSource = outboundRtp.getMediaSource();
      const trackId = outboundRtp.id  // outboundRtp.getTrackId();

      /*
      if (!trackId || !isValidUuid(trackId)) {
          logger.debug(`TrackId ${trackId} either not a uuid or does not
      exist`); continue;
      }
      */
      if (outboundRtp.stats.kind === 'audio') {
        /* eslint-disable @typescript-eslint/no-explicit-any */
        const {trackIdentifier, ...mediaSourceStats}: any =
            mediaSource ? (mediaSource.stats as W3C.RtcAudioSourceStats) : {};
        const {sfuStreamId} =
            this._trackRelations.get(trackId || 'notId') || {};
      ...
      if (outboundRtp.stats.kind === 'video') {
        const {trackIdentifier, ...videoSourceStats}: any =
            mediaSource ? (mediaSource.stats as W3C.RtcVideoSourceStats) : {};
        let mediaSourceStats = {};
        if (videoSourceStats) {
          if (!this._config.incrementalSampling)
            mediaSourceStats = videoSourceStats;
          else if (
              mediaSource && this._sampled &&
              this._sampled <= mediaSource.updated)
            mediaSourceStats = videoSourceStats;
        }
        const {sfuStreamId} =
            this._trackRelations.get(trackId || 'notId') || {};
        const {qualityLimitationDurations, ...outboundStats}:
  ...
  private *
      _makeInboundTrack(peerConnection: PeerConnectionEntry): Generator<
          [InboundAudioTrack | undefined, InboundVideoTrack|undefined], void,
          undefined> {
    ...
    for (const inboundRtp of peerConnection.inboundRtps()) {
      ...
      const trackId = inboundRtp.id  // inboundRtp.getTrackId();
      /*
      if (!trackId || !isValidUuid(trackId)) {
      ...
      */
      const remoteOutboundRtpStats =
          inboundRtp.getRemoteOutboundRtp()?.stats || {};
      const {sfuStreamId, sfuSinkId, remoteClientId} =
          this._trackRelations.get(trackId || 'notId') || {};
  ...
  private *
      _makeIceCandidatePair(peerConnection: PeerConnectionEntry):
          Generator<IceCandidatePair, void, undefined> {
    for (const entry of peerConnection.iceCandidatePairs()) {
      if (this._config.incrementalSampling && this._sampled &&
          entry.updated <= this._sampled) {
        continue;
      }
  ...
  private *
      _makePcTransport(peerConnection: PeerConnectionEntry):
  ...
  private *
      _makeMediaSource(peerConnection: PeerConnectionEntry):
          Generator<MediaSourceStat, void, undefined> {
    for (const mediaSourceEntry of peerConnection.mediaSources()) {
      ...
      const sample: MediaSourceStat = {
        ...(stats.kind === 'audio' ? (stats as W3C.RtcAudioSourceStats) :
                                     (stats as W3C.RtcVideoSourceStats)),
      };
      yield sample;
    }
  }
  ...
  private *
      _makeCodec(peerConnection: PeerConnectionEntry):
          Generator<MediaCodecStats, void, undefined> {
  ...
  private *
      _makeCertificate(peerConnection: PeerConnectionEntry):
          Generator<Certificate, void, undefined> {
  ...
  private *
      _makeIceLocalCandidate(peerConnection: PeerConnectionEntry):
  ...
  private *
      _makeIceRemoteCandidate(peerConnection: PeerConnectionEntry):
  ...
  private *
      _makeDataChannel(peerConnection: PeerConnectionEntry):
          Generator<DataChannel, void, undefined> {
  ...
  private *
      _makeIceServer(peerConnection: PeerConnectionEntry):
